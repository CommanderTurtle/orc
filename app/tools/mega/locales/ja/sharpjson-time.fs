module ConvertedFiles.Mega.Locales.Ja.TimeJson
...
let file = """{
  ...
  "crontabGuru": {
    "title": "Crontab Guru",
    "description": "cron式を日本語で解析・説明します。任意のcronスケジュールの動作を即座に理解し、構文を検証し、複数の式を一度に処理できます。",
    "shortDescription": "cron式をわかりやすく解析・説明する",
    "longDescription": "Crontab Guruは、複雑なcron構文をわかりやすい説明に変換するcron式パーサー・バリデーターです。自動バックアップ、システムメンテナンスタスク、CI/CDパイプラインのスケジュール設定など、ジョブがいつ実行されるかを正確に把握できます。標準的な5フィールドのcron式をサポートし、構文をリアルタイムで検証し、複数の式を一度に処理します（1行に1つ）。英語、フランス語、スペイン語、ドイツ語、日本語、中国語、ポルトガル語、オランダ語、ロシア語に対応しています。"
  },
  "timeBetweenDates": {
