module ConvertedFiles.Mega.Locales.Ru.TimeJson
...
let file = """{
  ...
  "crontabGuru": {
    "title": "Crontab Guru",
    "description": "Разбирайте и объясняйте cron-выражения на русском языке. Мгновенно понимайте, что делает любое cron-расписание, проверяйте синтаксис и обрабатывайте несколько выражений одновременно.",
    "shortDescription": "Разбор и объяснение cron-выражений простым языком",
    "longDescription": "Crontab Guru — это парсер и валидатор cron-выражений, который переводит сложный синтаксис cron в понятные человеку описания. Независимо от того, планируете ли вы автоматическое резервное копирование, задачи обслуживания системы или CI/CD-пайплайны, этот инструмент поможет вам точно понять, когда будут выполняться ваши задания. Поддерживает стандартные 5-польные cron-выражения, проверяет синтаксис в реальном времени и обрабатывает несколько выражений одновременно — по одному на строку. Доступно на английском, французском, испанском, немецком, японском, китайском, португальском, нидерландском и русском языках."
  },
  "timeBetweenDates": {
