...
export function formatFromBytes(bytes) {
    ...
    const ret = wasm.formatFromBytes(ptr0, len0);
    return __wbindgen_enum_Format[ret];
}
...
export function formatFromExtension(extension) {
    ...
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.formatFromExtension(ptr0, len0);
    return __wbindgen_enum_Format[ret];
}
...
export function formatFromPath(path) {
    ...
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.formatFromPath(ptr0, len0);
    return __wbindgen_enum_Format[ret];
}
 ...
 * @returns {Document}
 */
export function toDocument(bytes, format) {
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
        const ptr0 = passArray8ToWasm0(bytes, wasm.__wbindgen_export);
        const len0 = WASM_VECTOR_LEN;
        wasm.toDocument(retptr, ptr0, len0, isLikeNone(format) ? 13 : ((__wbindgen_enum_Format.indexOf(format) + 1 || 13) - 1));
 ...
 * @returns {string}
 */
export function toMarkdownBytes(bytes, format) {
    let deferred3_0;
    let deferred3_1;
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
...
const __wbindgen_enum_Format = ["doc", "docx", "odt", "pdf", "ppt", "pptx", "rtf", "epub", "xlsx", "ods", "odp", "csv"];
