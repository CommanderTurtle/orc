...
(function(self){
  ...
  function replaceStrings(){
    var regEx = /[^\[\]\(\)\!\+]{1}/g, all, value, missing, count = MAX - MIN;
    ...
    function mappingReplacer(a, b) { return b.split("").join("+"); }
    function valueReplacer(c) { return missing[c] ? c : MAPPING[c]; }
    for (all in MAPPING){
      if (MAPPING[all]){
  ...
  function encode(input, wrapWithEval, runInParentScope){
    ...
    if (!input) return "";
    var unmappped = '';
    for(var k in MAPPING) if (MAPPING[k]) unmappped += k;
    ...
    if (unmappedCharactersCount > 0) {
      output = "[][" + encode("at") + "][" + encode("constructor") + "](" + encode("return\"") + "+" + output + "+" + encode("\"") + ")()";
    }
    if (wrapWithEval){
      if (runInParentScope) {
        output = "[][" + encode("at") + "][" + encode("constructor") + "](" + encode("return eval") + ")()(" + output + ")";
      } else {
        output = "[][" + encode("at") + "][" + encode("constructor") + "](" + output + ")()";
      }
    }
    return output;
  }
